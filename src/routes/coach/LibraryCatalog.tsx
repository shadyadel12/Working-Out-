...
export default function LibraryCatalog({ kind }: { kind: LibraryKind }) {
  ...
  return <div className="catalog-page">
    ...
    {!query.isLoading && !query.error && rows.length === 0 && <div className="catalog-empty"><AppIcon name="library" size={34} /><h2>No {config.title.toLowerCase()} found</h2><p>Create the first reusable {config.singular}, or change the current search and status filters.</p><button onClick={() => setEditor('new')}>Create {config.singular}</button></div>}
...
function CatalogEditor({ kind, item, onClose, onSaved }: { kind: LibraryKind; item: CoachLibraryItem | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = useMutation({ mutationFn: () => saveCoachLibraryItem(kind, session!.user.id, item?.id ?? null, { title, summary, lifecycle, shareMode, subtype, numberValue, unit }), onSuccess: onSaved });
  ...
  return <div className="catalog-dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div ref={dialogRef} className="catalog-dialog" role="dialog" aria-modal="true" aria-labelledby={`${titleId}-heading`}>
    ...
    <form onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
      ...
      <footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button type="submit" disabled={!title.trim() || save.isPending}>{save.isPending ? 'Saving…' : `Save ${config.singular}`}</button></footer>
    </form>
  </div></div>;
}

function LibraryRelationsEditor({ kind, parentId, coachId }: { kind: LibraryKind; parentId: string; coachId: string }) {
  const supported = !['tasks','ingredients'].includes(kind); const [choiceId, setChoiceId] = useState(''); const [label, setLabel] = useState(''); const [subtype, setSubtype] = useState('short_text'); const [amount, setAmount] = useState(1); const [secondaryAmount, setSecondaryAmount] = useState(kind === 'sections' ? 60 : 1); const [unit, setUnit] = useState('g'); const [required, setRequired] = useState(false);
  ...
  const refresh = () => query.refetch(); const add = useMutation({ mutationFn: () => addLibraryRelation(kind, parentId, { choiceId, label, subtype, amount, secondaryAmount, unit, required }), onSuccess: async () => { await refresh(); setChoiceId(''); setLabel(''); } });
  ...
  return <fieldset className="catalog-relations"><legend>{relationHeading(kind)}</legend>
    ...
    <div className="catalog-relation-add">
      ...
      {kind === 'meal-plans' && <><label>Meal name<input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Breakfast" /></label><label>Week<input type="number" min="1" max="52" value={amount} onChange={(event) => setAmount(Number(event.target.value))} /></label><label>Day<input type="number" min="1" max="7" value={secondaryAmount} onChange={(event) => setSecondaryAmount(Number(event.target.value))} /></label></>}
