...
function CatalogEditor({ kind, item, onClose, onSaved }: { kind: LibraryKind; item: CoachLibraryItem | null; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [addUploadsToProgressPhotos,setAddUploadsToProgressPhotos]=useState(Boolean(item?.meta.add_uploads_to_progress_photos));
  const save = useMutation({ mutationFn: () => saveCoachLibraryItem(kind, session!.user.id, item?.id ?? null, { title, summary, lifecycle, shareMode, subtype, numberValue, unit, availableFrom, availableUntil, addUploadsToProgressPhotos }), onSuccess: onSaved });
  ...
  return <div className="catalog-dialog-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><div ref={dialogRef} className="catalog-dialog" role="dialog" aria-modal="true" aria-labelledby={`${titleId}-heading`}>
    ...
    <form onSubmit={(event) => { event.preventDefault(); save.mutate(); }}>
      ...
      {kind==='tasks'&&subtype==='progress_photo'&&<label className="catalog-check"><input type="checkbox" checked={addUploadsToProgressPhotos} onChange={event=>setAddUploadsToProgressPhotos(event.target.checked)}/> Add uploaded photos to Progress Photos</label>}
