      ...
      <div className="stack" style={{ borderTop: '1px solid var(--border)', paddingTop: '0.9rem' }}>
        ...
        {meals.length > 0 && <div className="diet-builder-grid">
          ...
          <aside className="diet-builder-inspector" aria-label="Meal details">
            {selected ? <><h3>{selected.label}</h3><p className="muted">Add foods from the library or enter a custom food.</p><RecipePicker coachId={coachId} onUse={(recipe,items)=>setMeals(current=>current.map((meal,index)=>index===selectedMeal?{...meal,items,recipe}:meal))} />{selected.recipe&&<div className="selected-recipe"><span><strong>{selected.recipe.title}</strong><small>{selected.recipe.ingredients.length} ingredients · {selected.recipe.servings} serving{selected.recipe.servings===1?'':'s'}</small></span><button type="button" className="secondary" onClick={()=>setMeals(current=>current.map((meal,index)=>index===selectedMeal?{...meal,recipe:null}:meal))}>Detach recipe</button></div>}{(selected.items ?? []).map((item, itemIndex) => {
