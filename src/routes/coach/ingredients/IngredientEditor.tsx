...
export default function IngredientEditor({
  ...
  const save = useMutation({
    mutationFn: async () => {
      const ids = multiple
        ? await saveIngredients(coachId, items)
        : [await saveIngredient(coachId, existing?.id ?? null, items[0])];
      if (!existing || existing.visibility !== visibility)
        await Promise.all(
          ids.map((id) => publishCatalogItem("ingredients", id, visibility)),
        );
    },
    onSuccess: onSaved,
  });
      ...
      <section className="catalog-dialog ingredient-dialog">
        <header>
          <div>
            ...
            <h2>{multiple ? "Multiple ingredients" : "Ingredient"}</h2>
          </div>
        ...
        >
            ...
            <div className="ingredient-form-row" key={index}>
              ...
              <label>
                ...
                <IngredientUnitSelect
                  ...
                  onChange={(defaultUnit) => patch(index, { defaultUnit })}
                />
              </label>
              <label>
                Image URL
                <input
                  ...
                  value={item.imageUrl}
                  onChange={(event) =>
                    patch(index, { imageUrl: event.target.value })
                  }
                  placeholder="https://…"
                />
              </label>
          ...
          {multiple && (
            <button
              ...
              className="secondary"
              onClick={() =>
                setItems((current) => [...current, blankIngredient()])
              }
            >
          ...
          <footer>
            ...
            <button
              disabled={
                items.some(
                  (item) => !item.name.trim() || !item.defaultUnit.trim(),
                ) || save.isPending
              }
            >
