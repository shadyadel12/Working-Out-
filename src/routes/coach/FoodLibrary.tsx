...
import SavedMealDays from './food-library/SavedMealDays';
...
export default function FoodLibrary() {
  ...
  const [view, setView] = useState<'foods' | 'days'>('foods');
  ...
  return <div className="food-library-page">
    <header className="catalog-heading"><div><span className="overview-kicker">Nutrition Library</span><h1>Foods</h1><p>Manage reusable foods and complete saved meal days.</p></div>{view === 'foods' && <button onClick={() => setAdding(true)}>+ Add food</button>}</header>
    <nav className="catalog-tabs food-library-tabs" aria-label="Food library sections"><button className={view === 'foods' ? 'active' : ''} onClick={() => setView('foods')}>Saved Foods</button><button className={view === 'days' ? 'active' : ''} onClick={() => setView('days')}>Saved Meal Days</button></nav>
    {view === 'days' ? <SavedMealDays coachId={coachId} /> : <>
