...
const blankExercise = (): DraftExerciseData => ({ name: '', target_sets: 3, target_reps: '10', target_weight: null, target_seconds: null, tempo: null, coach_comment: null, coach_video_url: null, coach_video_is_external: false });
      ...
      <section className="workout-modal player-training-builder" role="dialog" aria-modal="true" aria-labelledby="add-workout-title">
        ...
        <div className="workout-modal-body player-builder-body">
          ...
          <div className="player-builder-grid">
            ...
            <aside className="player-builder-inspector" aria-label="Exercise prescription">{selected ? <>
              <h3>Prescription</h3>
              <div className="field"><label>Exercise name</label><input value={selected.name} onChange={(event) => patchExercise(selectedExercise, { name: event.target.value })} placeholder="Chest Press" maxLength={160} /></div>
              <div className="field"><label>Prescription type</label><select value={selected.target_seconds ? 'time' : 'reps'} onChange={(event) => patchExercise(selectedExercise, event.target.value === 'time' ? { target_seconds: 60, target_reps: null } : { target_seconds: null, target_reps: '10' })}><option value="reps">Repetitions</option><option value="time">Timed exercise</option></select></div>
              <div className="modal-target-grid"><div className="field"><label>Sets</label><input type="number" min="1" max="100" value={selected.target_sets ?? ''} onChange={(event) => patchExercise(selectedExercise, { target_sets: event.target.value ? Number(event.target.value) : null })} /></div>{selected.target_seconds ? <div className="field"><label>Duration (seconds)</label><input type="number" min="1" max="86400" value={selected.target_seconds} onChange={(event) => patchExercise(selectedExercise, { target_seconds: Number(event.target.value) || null })} /></div> : <div className="field"><label>Reps</label><input value={selected.target_reps ?? ''} onChange={(event) => patchExercise(selectedExercise, { target_reps: event.target.value || null })} placeholder="8-12" /></div>}<div className="field"><label>Weight</label><input value={selected.target_weight ?? ''} onChange={(event) => patchExercise(selectedExercise, { target_weight: event.target.value || null })} placeholder="60 kg" /></div></div>
              {selected.target_seconds && <small className="muted">{selected.target_seconds >= 60 ? `${Math.floor(selected.target_seconds / 60)} min${selected.target_seconds % 60 ? ` ${selected.target_seconds % 60} sec` : ''}` : `${selected.target_seconds} sec`}</small>}
              <div className="field"><label>Tempo</label><input value={selected.tempo ?? ''} onChange={(event) => patchExercise(selectedExercise, { tempo: event.target.value || null })} placeholder="Example: 3-1-1-0" /></div>
              <div className="field"><label>Coach notes</label><textarea rows={3} value={selected.coach_comment ?? ''} onChange={(event) => patchExercise(selectedExercise, { coach_comment: event.target.value || null })} placeholder="Technique, rest, or other instructions" maxLength={5000} /></div>
              <VideoInput ownerId={playerId} value={{ url: selected.coach_video_url, isExternal: selected.coach_video_is_external }} onChange={(video) => patchExercise(selectedExercise, { coach_video_url: video.url, coach_video_is_external: video.isExternal })} />
            </> : <p className="muted">Select an exercise to edit it.</p>}</aside>
          </div>
