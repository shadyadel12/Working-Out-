...
export default function CoachPlanScreen() {
  ...
  async function base() {
    ...
    setPlayers(list);
    if (!playerId && list[0]) setPlayerId(list[0].id);
    setWorkouts(w.data ?? []);
    setDiets(d.data ?? []);
    setPrograms(g.data ?? []);
  }
  ...
  async function duplicate() {
    if (!selected) return;
    const target = (day + 1) % 7;
    try {
      const d = await ensure(target, selected.day_type);
      for (const w of selected.workouts ?? []) {
  ...
  return (
    ...
    >
      <Card>
        <Text style={textStyles.heading}>Player</Text>
      ...
      <View style={styles.wrap}>
        {(["schedule", "workout", "diet", "assign"] as Mode[]).map((x) => (
          <Chip
            key={x}
            active={mode === x}
            text={x}
...
function Schedule({
  ...
  value: any;
  create: (d?: number, t?: string) => Promise<any>;
  remove: () => void;
  duplicate: () => void;
}) {
  if (!value)
    return (
      <Card>
        ...
        <Button onPress={() => create()}>CREATE TRAINING DAY</Button>
        <Button secondary onPress={() => create(undefined, "rest")}>
          CREATE REST DAY
        </Button>
      </Card>
    );
  return (
    <Card>
      ...
      <Button secondary onPress={duplicate}>
        DUPLICATE DAY
      </Button>
...
function Workout({
  ...
  async function add() {
    if (!name.trim() || !exercise.trim()) return;
    try {
  ...
  return (
    <>
      ...
      {templates.map((t) => (
        <Button key={t.id} secondary onPress={() => saved(t.id)}>
          ADD SAVED: {t.name}
        </Button>
      ))}
    </>
  ...
  return (
    <>
      <Card>
        ...
        <Input value={grams} onChangeText={setGrams} placeholder="Grams" />
        <Input
          value={comment}
        ...
        />
        <Button onPress={save}>SAVE DIET DAY</Button>
      </Card>
      {templates.map((t) => (
        <Button key={t.id} secondary onPress={() => saved(t.id)}>
          ASSIGN SAVED: {t.name}
...
const styles = StyleSheet.create({
  ...
  week: { color: colors.text, fontWeight: "900" },
  block: {
    borderTopWidth: 1,
