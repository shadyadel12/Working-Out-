...
Deno.serve(async(request)=>{
 ...
 const{data:run,error:runError}=await admin.from('external_catalog_sync_runs').insert({provider,requested_by:user.id,requested_count:limit}).select('id').single();if(runError)return json({error:runError.message},500);
 ...
 try{
  ...
  for(const raw of records.slice(0,limit)){
   const externalId=String(raw.id??raw.fdcId??raw.code??'');const name=clean(raw.name??raw.description??raw.product_name);
   ...
   if(provider==='wger'){
    const licensed=Boolean(raw.license?.full_name||raw.license?.short_name);const image=licensed?((raw.images??[]).find((x:any)=>x.is_main)?.image??raw.images?.[0]?.image??null):null;
    payload={coach_id:ownerId,name,category:clean(raw.category?.name)||'Exercise',equipment:(raw.equipment??[]).map((x:any)=>clean(x.name)).filter(Boolean).join(', ')||null,instructions:clean(raw.description),target_muscle_groups:[...(raw.muscles??[]),...(raw.muscles_secondary??[])].map((x:any)=>clean(x.name)).filter(Boolean),movement_patterns:[],tracking_fields:['Sets','Reps'],image_url:image,visibility:'public',lifecycle:'published',creator_name:'Trainova Catalog',source_provider:provider,external_id:externalId,source_url:`https://wger.de/en/exercise/${externalId}/view`,source_license:clean(raw.license?.full_name??raw.license?.short_name??source.license_name),source_attribution:clean(raw.license_author)||source.attribution,imported_at:new Date().toISOString(),last_synced_at:new Date().toISOString(),sync_status:'ok',moderation_status:'visible'};table='exercise_library';
   }else{
